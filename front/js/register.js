var vm = new Vue({
	...
	methods: {
		...
		check_username: function (){
			var _this = this;
			var len = _this.username.length;
			if(len<5||len>20) {
				_this.error_name_message = '请输入5-20个字符的用户';
			...
			// 检查用户名的唯一性
			if (_this.error_name == false) {
				axios.get(this.host+'/usernames/' + _this.username + '/count/', {
						responseType: 'json'
					})
					.then(response => {
					    // axios中，后端返回的数据会全部保存在 response.data
						if (response.data.count > 0) {
							_this.error_name_message = '用户名已存在';
							_this.error_name = true;
						} else {
							_this.error_name = false;
							_this.error_name_message = '请输入5-20个字符的用户';
